...
cat >> /home/viperuser/projects/ZenClarity-UrbanFlow-V2/docs/decisions/decisions_log.md << 'EOF'

---

## Decision 16: Parallel Warehouse Architecture - Same S3 Source vs Copy

**Date:** April 2026

**Context:** Needed to benchmark Snowflake vs Redshift on the same
dataset. Two options - copy data into each warehouse separately,
or point both engines at the same S3 Iceberg files via Glue Catalog.

**Options considered:**
- Copy data into each warehouse - simple setup, guaranteed isolation,
  but introduces data drift risk and storage cost on both engines
- Same S3 Iceberg source via Glue Catalog - zero copy, no drift,
  true apples-to-apples benchmark

**Decision:** Same S3 source. Both Snowflake and Redshift read the
same physical parquet files via Glue Catalog - no data movement.
Snowflake uses External Volume + Catalog Integration.
Redshift uses Spectrum + external schema.
Neither warehouse owns the data.

**Impact:** Benchmark measures engine performance only - not data
differences. 41,169,300 records confirmed identical via COUNT(*)
on both engines. Storage cost for external layer is zero on both.
Full benchmark documented in docs/benchmark/.

---

## Decision 17: Staging Materialization - View vs Incremental Table on Redshift

**Date:** April 2026

**Context:** Redshift Spectrum does not support creating views over
external tables. On Snowflake, staging models are zero-cost views
that resolve directly to S3 Iceberg at query time. The same pattern
fails on Redshift - first attempted run returned:
"External tables are not supported in views."

**Options considered:**
- Split into separate Snowflake and Redshift model files - clean
  per-engine code but breaks single codebase portability story
- Jinja conditional in config block - one file, engine-specific
  materialization at build time
- Materialize staging as a full table on Redshift - simple but
  rebuilds 41M records on every run

**Decision:** Jinja conditional config - incremental table on Redshift,
view on Snowflake. Single codebase maintained. Incremental strategy
uses merge on composite unique_key with ingestion_ts watermark filter
so subsequent runs only process new records - not a full 41M reload.
sort key on ingestion_ts matches the incremental filter access pattern.

**Impact:** Single dbt codebase confirmed working on both engines —
67/67 models and tests passing. Staging build cost on Redshift is
~6s full load, ~5s incremental. Snowflake staging is zero cost (view).
Benchmark queries run against mart layer only - both engines query
physical materialized tables at query time - apples-to-apples
comparison preserved.
Six models required engine-specific syntax fixes via target.type
Jinja conditionals - documented in docs/benchmark/02_materialization_decisions.md.

---

## Decision 18: Benchmark Query Layer - Mart Only

**Date:** April 2026

**Context:** Staging materializes differently across engines —
Snowflake uses zero-cost views, Redshift uses incremental tables.
Question arose on whether this makes the benchmark unfair.

**Options considered:**
- Benchmark at staging layer - directly measures external table
  read performance but not apples-to-apples due to materialization difference
- Benchmark at mart layer - both engines query physical materialized
  tables, true apples-to-apples on query performance

**Decision:** Benchmark queries run against mart layer only.
fact_trip, dim_taxi_zone, and dim_date are physical materialized
tables on both engines by the time queries run. The staging
materialization difference affects build time only - not query
performance. This distinction documented explicitly in
docs/benchmark/01_parallel_architecture.md.

**Impact:** Five benchmark queries confirmed apples-to-apples.
Build time difference (14.65s Snowflake vs 36.93s Redshift)
documented separately as an operational cost metric - not a
query performance metric. Cold start penalty on Redshift (~25s
on first query after idle) identified as the primary performance
differentiator, not query execution speed.
Full results in docs/benchmark/04_benchmark_results.md.
EOF
