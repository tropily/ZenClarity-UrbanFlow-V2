...
def align_to_iceberg_schema(df, limit_rows: Optional[int] = None):
    ...
    aligned = (
        ...
        .select(
            ...
            # === FULL METADATA BLOCK (Matching Athena Describe) ===
            F.current_timestamp().cast("timestamp").alias("ingestion_ts"),
            F.lit(BATCH_ID).alias("batch_id"),
            F.lit("MANUAL_S3_RECOVERY").alias("source_file"),
...
def write_to_stage(unified_df):
    """
    Ensures columns are in the EXACT order defined in the Iceberg table schema.
    """
